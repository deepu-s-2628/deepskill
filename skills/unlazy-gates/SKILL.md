...
`unlazy`'s native behavior for an unsatisfiable gate is `ABANDON: <id> <reason>`, which makes the checker exit `1` with `HANDOFF REQUIRED` — a stop signal, built for a human-supervised session. **`deepskill`'s pipeline never pauses except at its two sanctioned checkpoints** (wayfinding, and the Step 6 opt-in choice — see `context/pm-operating-system.md` §8). A quality gate is neither of those, so `ABANDON`/handoff semantics are never used here. Instead:
