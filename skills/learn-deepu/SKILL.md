...
Before researching a topic, resolve `$CLAUDE_PLUGIN_ROOT` (e.g. `echo $CLAUDE_PLUGIN_ROOT` via Bash) if not already known this session, then check whether the topic actually relates to OpManager Plus/Nexus or ITOM concepts — cross-reference `$CLAUDE_PLUGIN_ROOT/context/product-context.md`'s feature catalog and `$CLAUDE_PLUGIN_ROOT/context/CONTEXT.md`'s glossary.
