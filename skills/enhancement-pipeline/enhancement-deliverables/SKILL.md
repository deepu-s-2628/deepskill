...
- [ ] G1: All four Step 5 drafts written under `.steps/` with correct filenames for this pipeline mode
  CHECK: node -e "const fs=require('fs');const f=['.steps/5a_executive_presentation.md','.steps/5b_engineering_presentation.md','.steps/5c_enhancement_prd.md','.steps/5d_enhancement_flowchart.md'];process.exit(f.every(p=>fs.existsSync(p))?0:1)"
  EXPECT: (exits zero)
...
- [ ] G4: No TBD/lorem/fill-in-later placeholders in any of the four drafts
  CHECK: node -e "const fs=require('fs');const f=['.steps/5a_executive_presentation.md','.steps/5b_engineering_presentation.md','.steps/5c_enhancement_prd.md','.steps/5d_enhancement_flowchart.md'];process.exit(f.some(p=>/\bTBD\b|\blorem\b|\[fill in\]/i.test(fs.readFileSync(p,'utf8')))?1:0)"
  EXPECT: (exits zero)
...
- [ ] G7: `Progress.md` updated (next: document generation)
  CHECK: node -e "const fs=require('fs');process.exit(fs.statSync('Progress.md').mtimeMs>=fs.statSync('.steps/5d_enhancement_flowchart.md').mtimeMs?0:1)"
  EXPECT: (exits zero)
## Completion
...
>
> Drafts written under `[feature-name]-enhancement/.steps/` (`5a`–`5d`), `analysis.html` rebuilt.
