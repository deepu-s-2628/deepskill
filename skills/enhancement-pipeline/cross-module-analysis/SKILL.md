...
Reference [context/product-context.md]($CLAUDE_PLUGIN_ROOT/context/product-context.md) and identify every module that touches or relates to this feature:
...
Gate ledger for this step — write `.steps/GATES-2.md` from `$CLAUDE_PLUGIN_ROOT/skills/unlazy-gates/templates/gates-leaf.md` before producing this step's content, one gate per item below. Resolution rule (self-correct on a failed gate, document the gap and continue — never abandon, never pause): `$CLAUDE_PLUGIN_ROOT/context/pm-operating-system.md` §18.
