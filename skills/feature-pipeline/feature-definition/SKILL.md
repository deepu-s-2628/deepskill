...
- [context/product-context.md](pm-shared/context/product-context.md) — existing product patterns
...
Gate ledger for this step — write `.steps/GATES-4.md` from `<resolved shared parent>/unlazy-gates/templates/gates-leaf.md` before producing this step's content, one gate per item below. Resolution rule (self-correct on a failed gate, document the gap and continue — never abandon, never pause): `pm-shared/context/pm-operating-system.md` §18.
