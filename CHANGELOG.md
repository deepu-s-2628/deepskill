...
- [`a4b4401`](https://github.com/deepu-s-2628/deepskill/commit/a4b4401bbb1c6a37fdeeb084cafb0f7f8d3708fd) Thanks [@deepu-s-2628](https://github.com/deepu-s-2628)! - The plain-language bar (`pm-operating-system.md` §7) is now the default writing style for every pipeline step's output, not an on-demand fallback a PM has to trigger via `wait-what` after already being confused. Rewrote §7 to state this explicitly and name the connection to `wait-what`'s own "plain words, no unexplained jargon" standard, already coined but only applied to document-generation section intros before.
